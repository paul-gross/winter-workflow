...
This document is used by the blizzard architect agent during the **Principles Bootstrap Workflow**. When a project has no documented architectural principles, the architect proposes these as defaults. Each principle is spelled out for agent consumption — not just named, but explained with what it means in practice and how agents should apply it.
