...
- **Running tests, services, or builds** — that's for verifiers and the caller's workspace service tooling.
...
- **notes** — Brief acknowledgments (optional, keep short) of harness/application moves the change gets right (e.g., "added a `--debug` flag the workspace service tooling can consume; good"), plus any out-of-scope routing ("structural concern in `foo.py`; defer to `cold-reviewer`").
