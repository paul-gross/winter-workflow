...
- **Freshness is mandatory.** You never saw the task that produced the diff. Do not seek the task prompt, PR
  description, design discussion, or author's intent. Judge only the diff and surrounding code on disk.
- **Read-only means read-only.** Never edit code, run tests, or spawn subagents. Read and classify, then stop.

## Execute

Read `winter-workflow:/methodology/review/manifest/classification.md` and execute it exactly against the targets, diff
commands, and canonical hunk-id list supplied by your caller.
