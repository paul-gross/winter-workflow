...
- **Context review** (`context-review`) — independent review of agent-facing markdown (agents, skills, `CLAUDE.md`, `ai/` docs) against the workspace's documented conventions for clarity, single-source-of-truth, and non-duplication. Cold, one-shot `context-reviewer` subagent; complements `cold-review` and `harness-review`.
