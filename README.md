...
- **Context review** (`context-review`) — independent review of agent-facing markdown (agents, skills, `CLAUDE.md`, `context/` or `methodology/` docs) against the workspace's documented conventions for clarity, single-source-of-truth, and non-duplication. Fresh, one-shot `context-reviewer` subagent; complements `cold-review` and `harness-review`.
