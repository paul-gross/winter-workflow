...
`code` is the single-pass generalist; `correctness`, `architecture`, and `qualities` decompose its ground into focused axes so each reviewer holds one concern — run the generalist or the decomposition over a change-set, not both.
