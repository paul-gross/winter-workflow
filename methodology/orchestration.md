...
| `plan-review` | Run glacier's **plan-review gate** over the plan: the verifiability gate (every planned change maps to a declared verification method, or schedules building a missing one) and the architecture gate (the plan conforms to the architecture guidance). |
