...
|-----------|------------------------|
| `plan` | Execute the [planning process](./planning/process.md): target the application's **verifiability matrix** and **architecture guidance**, conform to [plan-shape](./planning/plan-shape.md), and spawn an isolated `arctic-explorer` for codebase research rather than reading it all into your own context. On a re-entry after a failed gate, execute its revision step — deletion first, growth only with a per-addition justification naming the finding that required it. The graph owns the loop. |
| `plan-review` | Run the **plan-review gate**: execute the shared [review process](./review/process.md) with `axis: plan`, the chunk's plan as the paths scope, and the chunk's work-target path(s) as `work_target`. The [`plan` axis](./review/axes/plan.md) owns what the gate checks. A chunk's plan lives in a graph asset, not a framework's work-item layout, so name the [planning process](./planning/process.md) as the plan's owning framework in the scaffold — its [plan-shape](./planning/plan-shape.md) conventions are the conformance gate's governing spec. |
...
| `review` | Run the [faceted review](./review/faceted/process.md): a review worker is cold by construction, so evaluate its lead-placement rule from that seat — when the rule's remaining conditions hold, **you are the lead**, forking per facet in your own context. Name facets from the [axes registry](./review/axes/index.md) to cover the node's required axes and any registered axis whose concern the change-set touches; [reporting](./review/reporting.md) severities structure the findings asset. |
