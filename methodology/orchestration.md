...
| `plan-review` | Run the **plan-review gate**: execute the shared [review process](./review/process.md) with `axis: plan`, the chunk's plan as the paths scope, and the chunk's work-target path(s) as `work_target`. The [`plan` axis](./review/axes/plan.md) owns what the gate checks. |
