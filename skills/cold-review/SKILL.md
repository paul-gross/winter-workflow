...
On a large or mechanical-heavy diff, consider generating a [review manifest](winter-workflow:/ai/review-manifest/index.md) first (the `review-manifest` skill) — it tiers every hunk and renders a review order (`novel` first in full, `pattern`/`mechanical` collapsed) that a human reads to focus their own review, and that can also order this cold review's attention onto the decisions instead of every line equally. Optional and advisory; skip it on a small diff.
