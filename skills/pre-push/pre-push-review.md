...
> **Optional pre-step — a review manifest.** When the un-pushed change-set is large or mechanical-heavy, generate a [`winter-workflow:/context/review-manifest/index.md`](winter-workflow:/context/review-manifest/index.md) over it first — follow [`winter-workflow:/context/review-manifest/pipeline.md`](winter-workflow:/context/review-manifest/pipeline.md) in `unpushed` scope. It tiers every hunk and renders a review order (`novel` first in full, `pattern`/`mechanical` collapsed), so you can focus your read of the reviewers' findings on the `novel` hunks. Advisory and independent of the fan-out below — skip it on a small change-set.
