...
If services aren't running and the verification needs them, the developer should say so rather than guess — glacier does not start services. Tell the user to run `./up` (or the project equivalent per `workspace:/context/project/project-setup.md`) and re-invoke.
