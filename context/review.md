# Review engine — one fresh review, every axis and scope
...
This engine produces *findings*; it does not shrink the surface a human must read. A [**review manifest**](./review-manifest/index.md) does — it partitions every hunk of the same change-set into verification tiers (`mechanical` / `pattern` / `novel`) and renders a review order that a **human reviewer** reads so full attention lands on the `novel` hunks. It is an **optional preprocessing step**, not a review axis, and its reader is a human first: run [`review-manifest`](../skills/review-manifest/SKILL.md) on a large or mechanical-heavy diff to get a review order — read it to focus your own review, and optionally generate it before a fresh review to pre-order that too. On a small diff that fits in a glance, skip it. Advisory; gates nothing.
...
- **A review skill** — `cold-review` (code), `context-review` (context), `harness-review` (harness), `documentation-review` (documentation). Each binds one axis and passes `$ARGUMENTS` through as the scope. Fresh subagent by default; runs inline when `$ARGUMENTS` leads with the `inline` selector.
...
- **`pre-push`** — composes several axes in parallel over the un-pushed change-set; see [`./pre-push/process.md`](./pre-push/process.md), which references this doc for the scaffold and per-axis bodies rather than restating them.
...
|------|----------------|---------|
| `code` | [`cold-reviewer`](../agents/cold-reviewer.md) | architectural quality, design-principle adherence |
...
|-------|----------|-----------|-------------------------|
| **branch-vs-base** (default) | _(none)_ | env-wide — [`./changeset-scope.md`](./changeset-scope.md) | per changeset-scope's mode table (in-scope predicate + diff base) |
| **uncommitted** | `uncommitted` | env-wide — [`./changeset-scope.md`](./changeset-scope.md) | per changeset-scope's mode table (incl. untracked files) |
...
| **unpushed** | _(`pre-push` only)_ | env-wide — [`./changeset-scope.md`](./changeset-scope.md) | per changeset-scope's mode table (the `winter ws push` predicate) |
...
Categorized findings:
   ...
   Give every `must-fix` and `consider` finding a **distinct id**: prefix `M` for must-fix, `C` for consider, numbered by **one sequence across both sections** — the numbering continues from must-fix into consider, never restarting (e.g. two must-fixes and two considers are `M1, M2, C3, C4`, not `C1, C2`). A number is never reused across the report, so any finding can be cited unambiguously by id in follow-up discussion and fixes.
...
> Follow your agent body (`cold-reviewer`). Read the changed files and surrounding code for context (existing patterns, conventions); eagerly load coding standards, architecture docs, and any in-flight initiatives. No extra caller context is needed.

### context

> Follow your agent body (`context-reviewer`). Load the harness conventions for agent-facing markdown the workspace exposes, and any `context/` docs governing the touched files. Check naming/prefixes, path notation, voice, frontmatter, cross-reference freshness, and single-source-of-truth.
...
Each reviewer is **role-pure and team-less** (see [`../agents/README.md`](../agents/README.md)): no shared `TaskList`, no peers, no follow-on. This keeps every review composable — a user invokes it directly, an `iceberg` foreman invokes it as a contained sub-step, `pre-push` fans several out in parallel, and no reviewer ever tries to coordinate work it isn't responsible for.
