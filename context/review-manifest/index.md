...
| [`cold-review`](../../skills/cold-review/SKILL.md) | a change-set | a fresh code review the manifest can also focus |
| [`pre-push`](../pre-push/process.md) | the un-pushed change-set | a multi-axis gate the manifest can also pre-order |
...
| [`./pipeline.md`](./pipeline.md) | You are generating a manifest **by fresh classification** of a finished diff — the classify → audit → render control flow |
...
This cut covers both producers (fresh classification and build-time intent capture), the adversarial audit, and the render. Two verifications named in the tier table are deliberate follow-ups, not built here:
