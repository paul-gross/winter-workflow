...
3. **Audit the cheap tiers.** Run the [`manifest-auditor`](../../agents/manifest-auditor.md) over the `authored` `mechanical` and `pattern` hunks exactly as in [`./pipeline.md`](./pipeline.md) step 3. The auditor is fresh and adversarial; it does not get your intent as a free pass. Hits promote to `novel` and increment `misclassification_count`.
...
| | Fresh classification ([`pipeline.md`](./pipeline.md)) | Build-time intent capture (this doc) |
|---|---|---|
