...
The **build skills** satisfy the *tested* bar through their own runtime-probe flows and reference this convention rather than restating it: `glacier` and `blizzard` verify each change at runtime and run a pre-push review spanning the code, agent-facing, and public-docs axes; `thaw`, scoped to small localized changes, verifies at runtime and surfaces the owed delivery surfaces in its report for the user to carry. Those flows currently route to ad-hoc probes rather than consulting a declared verifiability matrix first; aligning them to read the matrix is follow-up work.
